    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button onClick={() => {
            if (!drivers.length) { toast.error('No data to export'); return; }
            const esc = (v: any) => `"${String(v ?? '').replace(/"/g, '""')}"`;
            const header = 'Name,Email,Vehicle Type,Vehicle Plate,License,Verified,Rating,Total Rides\n';
            const rows = drivers.map((d) => [esc(d.User?.name), esc(d.User?.email), esc(d.vehicle_type), esc(d.vehicle_plate), esc(d.license_number), esc(d.is_verified ? 'Yes' : 'No'), esc(d.rating ?? ''), esc(d.total_rides ?? '')].join(',')).join('\n');
            const blob = new Blob([header + rows], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = `omji-drivers-${new Date().toISOString().slice(0, 10)}.csv`;
            link.click();
          ...
          <table className="w-full">
            <thead className="bg-gray-50/80 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">License</th>
