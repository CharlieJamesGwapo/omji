    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button onClick={() => {
            if (!rides.length) { toast.error('No data to export'); return; }
            const esc = (v: any) => `"${String(v ?? '').replace(/"/g, '""')}"`;
            const header = 'ID,Pickup,Dropoff,Vehicle Type,Fare,Status,Date\n';
            const rows = rides.map((r) => [esc(r.id), esc(r.pickup_location), esc(r.dropoff_location), esc(r.vehicle_type), esc(r.final_fare ?? r.estimated_fare ?? ''), esc(r.status), esc(r.created_at ? new Date(r.created_at).toLocaleDateString() : '')].join(',')).join('\n');
            const blob = new Blob([header + rows], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = `omji-rides-${new Date().toISOString().slice(0, 10)}.csv`;
            link.click();
