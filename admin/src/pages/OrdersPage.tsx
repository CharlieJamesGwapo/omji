      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button onClick={() => {
            ...
            const header = 'ID,Store,Total,Status,Payment Method,Date\n';
            const rows = orders.map((o) => [esc(o.id), esc(o.Store?.name), esc(o.total_amount), esc(o.status), esc(o.payment_method), esc(o.created_at ? new Date(o.created_at).toLocaleDateString() : '')].join(',')).join('\n');
            const blob = new Blob([header + rows], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = `omji-orders-${new Date().toISOString().slice(0, 10)}.csv`;
            link.click();
