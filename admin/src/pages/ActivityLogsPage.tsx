    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">{filteredLogs.length} activities recorded</p>
        </div>
        <input
          type="text"
          ...
          className="w-full sm:w-80 px-4 py-3 border-2 border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-red-600 focus:border-red-600"
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
          <div className="text-3xl font-bold">{logs.filter((l) => l.action === 'USER_REGISTERED').length}</div>
          <div className="text-blue-100 text-sm mt-1">User Registrations</div>
        </div>
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white shadow-lg">
          <div className="text-3xl font-bold">{logs.filter((l) => l.action === 'DRIVER_REGISTERED').length}</div>
          <div className="text-purple-100 text-sm mt-1">Driver Registrations</div>
        </div>
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg">
          <div className="text-3xl font-bold">
            {logs.filter((l) => l.action === 'USER_VERIFIED' || l.action === 'DRIVER_VERIFIED').length}
          </div>
          <div className="text-green-100 text-sm mt-1">Verifications</div>
        </div>
        <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-6 text-white shadow-lg">
          <div className="text-3xl font-bold">{logs.length}</div>
          <div className="text-red-100 text-sm mt-1">Total Activities</div>
        </div>
      </div>
            ...
            filteredLogs.map((log) => (
              <div key={log.id} className="p-6 hover:bg-gray-50 transition-colors">
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-lg ${getActionColor(log.action)}`}>{getActionIcon(log.action)}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-medium text-gray-900 mb-1">{log.description}</p>
