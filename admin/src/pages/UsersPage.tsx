...
import { ITEMS_PER_PAGE, ROLE_BADGE_CLASSES, VERIFIED_BADGE, UNVERIFIED_BADGE } from '../constants';
...
const UsersPage: React.FC = () => {
  ...
  if (loading) {
    return <PageSkeleton statCards={4} filterButtons={6} tableRows={8} />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <button onClick={() => {
            ...
            const header = 'Name,Email,Phone,Role,Verified,Rating,Joined Date\n';
            const rows = users.map((u) => [esc(u.name), esc(u.email), esc(u.phone), esc(u.role), esc(u.is_verified ? 'Yes' : 'No'), esc(u.rating ?? ''), esc(u.created_at ? new Date(u.created_at).toLocaleDateString() : '')].join(',')).join('\n');
            const blob = new Blob([header + rows], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = `omji-users-${new Date().toISOString().slice(0, 10)}.csv`;
            link.click();
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${
                    user.is_verified ? VERIFIED_BADGE : UNVERIFIED_BADGE
                  }`}>
      ...
      {/* User Details Modal */}
      <Modal
        open={showDetailModal && !!selectedUser}
          ...
          <>
            ...
            <div className="flex items-center gap-3 sm:gap-4 -mt-2 mb-4">
              ...
              <button
                ...
                className="ml-auto text-gray-400 hover:text-gray-600 hover:bg-gray-100 p-2 rounded-lg transition-colors min-w-[40px] min-h-[40px] flex items-center justify-center"
                aria-label="Edit user"
              >
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap mb-4">
              <span className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium ${
                selectedUser.is_verified ? VERIFIED_BADGE : UNVERIFIED_BADGE
              }`}>
