...
import { SERVICE_LABELS, SERVICE_COLORS, VEHICLE_LABELS } from '../constants';
import { getErrorMessage } from '../utils';
import { ConfirmDialog, EmptyState, PageSkeleton, Modal } from '../components';
...
const emptyForm: Partial<RateConfig> & { description?: string } = {
  service_type: 'ride',
...
export default function RatesPage() {
  const [rates, setRates] = useState<(RateConfig & { description?: string })[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingRate, setEditingRate] = useState<RateConfig | null>(null);
  const [form, setForm] = useState<Partial<RateConfig> & { description?: string }>(emptyForm);
  const [saving, setSaving] = useState(false);
  const [deletingId, setDeletingId] = useState<number | null>(null);
  const [confirmDialog, setConfirmDialog] = useState<{ open: boolean; rateId: number | null }>({ open: false, rateId: null });
  ...
  const fetchRates = async () => {
    try {
      ...
      toast.error(getErrorMessage(err, 'Failed to load rates'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (Number(form.base_fare) < 0 || Number(form.rate_per_km) < 0 || Number(form.minimum_fare) < 0) {
      toast.error('Fare values cannot be negative');
      return;
    }
    if (Number(form.base_fare) === 0 && Number(form.rate_per_km) === 0) {
      toast.error('Base fare or rate per km must be greater than 0');
      return;
    }
    setSaving(true);
    try {
  ...
  const handleDeleteConfirm = async () => {
    if (!confirmDialog.rateId) return;
    setDeletingId(confirmDialog.rateId);
    try {
      await adminService.deleteRate(confirmDialog.rateId);
      toast.success('Rate deleted');
      fetchRates();
    } catch (err) {
      toast.error(getErrorMessage(err, 'Failed to delete rate'));
    } finally {
      setDeletingId(null);
      setConfirmDialog({ open: false, rateId: null });
    }
  };
  ...
  if (loading) {
    return <PageSkeleton statCards={4} tableRows={3} showSearch={false} />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {rates.length === 0 ? (
          <EmptyState
            ...
            description="Default rates are being used. Add a custom rate to override defaults."
            action={{ label: 'Add First Rate', onClick: openCreate }}
          />
        ) : (
          <>
            {/* Desktop table */}
                      ...
                      <tr key={rate.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-5 py-3.5 text-center">
                          <button
                            ...
                            aria-label={rate.is_active ? 'Deactivate rate' : 'Activate rate'}
                          >
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => openEdit(rate)} className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" aria-label="Edit rate">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                              </svg>
                            </button>
                            <button
                              onClick={() => setConfirmDialog({ open: true, rateId: rate.id })}
              ...
              {rates.map(rate => {
                ...
                return (
                  <div key={rate.id} className="p-4">
                    ...
                    {(rate as any).description && <p className="text-xs text-gray-500 mb-3">{(rate as any).description}</p>}
                    <div className="flex items-center gap-2">
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to delete this rate configuration? This action cannot be undone."
        confirmLabel="Delete"
        variant="danger"
        onConfirm={handleDeleteConfirm}
        onCancel={() => setConfirmDialog({ open: false, rateId: null })}
      />
